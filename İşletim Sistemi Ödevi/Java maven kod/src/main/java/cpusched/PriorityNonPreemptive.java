...
public class PriorityNonPreemptive implements Scheduler {
    @Override public String getName(){return "Priority-NonPreemptive";}
    @Override
    public SchedulerResult run(List<ProcessInfo> input) {
        ...
        while (idx < procs.size() || !pq.isEmpty()) {
            ...
            if (pq.isEmpty()) {
                if (idx < procs.size()) { double nextArr = procs.get(idx).arrival; res.timeline.add(new TimelineEntry(time, nextArr, "IDLE")); time = nextArr; continue; }
                break;
            }
            ProcessInfo p = pq.poll();
            p.startTime = time; time += p.burst; p.finishTime = time; p.waitingTime = p.startTime - p.arrival; res.timeline.add(new TimelineEntry(p.startTime, p.finishTime, p.id)); res.processes.put(p.id,p);
        }
